...
class CVFilter:
    ...
    def predict_state_covariance(self, delt, plant_noise):
        ...
        Pp = np.dot(np.dot(self.Phi, self.pf), self.Phi.T) + Q
        return self.Sp, Pp
...
for measurement in measurements:
    Sp, Pp = kalman_filter.predict_state_covariance(measurement[3] - kalman_filter.Meas_Time, np.eye(6))
